...
class NodeList implements \Iterator, \Countable {
	...
	public function rewind() {
		if(!$this->hasResults())
			throw new EmptyResultException('XPath error: '.$this->search.' not found.');
		reset($this->result);
    }
	...
	public function select($search) {
		...
		if($firstItem) return $firstItem->select($search);
		else throw new EmptyResultException('XPath error: '.$this->search.' not found.');
	}
	
	public function extract() {
		$firstItem=$this->item(0);
		if($firstItem) return $firstItem->extract();
		else throw new EmptyResultException('XPath error: '.$this->search.' not found.');
	}
